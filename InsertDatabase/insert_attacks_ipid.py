...
# Create a mapping from IPID value to packet_id (from captured.json)
ipid_to_packet_id = {pkt.get("IPID"): pkt.get("packet_id") for pkt in packets}

# Filter attacks: only keep those where modified_field == "IPID" and modified_value matches a captured packet's IPID
matching_attacks = []
for atk in attacks:
    mv = atk.get("modified_value")
    mfield = str(atk.get("modified_field", "")).upper()

    if mfield == "IPID" and mv in ipid_to_packet_id:
        # Add the actual packet_id from captured.json
        atk["packet_id_from_captured"] = ipid_to_packet_id[mv]
        matching_attacks.append(atk)

print(f"Total attacks in attack_log.json: {len(attacks)}")
print(f"Matching attacks (modified_field == IPID and modified_value in captured.json IPID): {len(matching_attacks)}")
...
if not matching_attacks:
    print("No matching IPID attacks found. Exiting.")
else:
    # -----------------------
    ...
    for i, atk in enumerate(unique_attacks, start=1):
        ...
        values = (
            "TCP IPID Field Modification",  # AttackName
    ...
    print(f"Inserted/updated {len(unique_attacks)} matching IPID attacks successfully!")
