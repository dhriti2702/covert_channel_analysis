...
def is_ack_covert(pkt):
    """Return True if all bytes of ACK field are printable ASCII characters (32..126)."""
    try:
        ack = pkt[TCP].ack
    except Exception:
        return False
    
    # Extract 4 bytes from 32-bit ACK value
    b3 = (ack >> 24) & 0xFF
    b2 = (ack >> 16) & 0xFF
    b1 = (ack >> 8) & 0xFF
    b0 = ack & 0xFF
    ...
    return (32 <= b3 <= 126) and (32 <= b2 <= 126) and (32 <= b1 <= 126) and (32 <= b0 <= 126)
...
def extract_packet_entry(pkt):
    ...
    entry = {
        ...
        "Label": "covert" if is_ack_covert(pkt) else "normal"
    }
...
def pkt_callback(pkt):
    ...
    # check ACK covert condition
    if not is_ack_covert(pkt):
        return
    ...
    print(f"[ACK COVERT] packet_id={entry['packet_id']} ACK={entry['ack']:08x}")


if __name__ == "__main__":
    print("Listening for outgoing ACK-based covert packets...")
